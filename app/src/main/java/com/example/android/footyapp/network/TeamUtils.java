        ...
        try{
            ...
            for(int j = 0; j < standings.length(); j++){
                ...
                if( row.getString("teamName").equals(team)) {
                    ...
                    teamObj = new Team(row.getString("teamName"), row.getString("crestURI"),  row.getString("playedGames"), row.getString("points"),  row.getString("wins"),
